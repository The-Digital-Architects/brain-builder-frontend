            ...
            <GridBox>
                {introsByLevel[level] && introsByLevel[level].map((intro, index) => {
                    ...
                    return entry && entry.visibility ? (
                        <ChallengeButton key={`intro${level}${intro}_button`} link={`introduction${level}${intro}`} label="Introduction" Icon={RocketIcon} active={progressData["intros"]?.[levelStr]?.[index] !== undefined ? progressData["intros"]?.[level]?.[index] : false} />
                    ) : null;
                })}

                {challenges.map((challenge, index) => (
                    <ChallengeButton key={`challenge${level}${challenge}_button`} link={`challenge${level}${challenge}`} label={taskNames[`${level}${challenge}`]} Icon={RocketIcon} active={progressData["challenges"]?.[levelStr]?.[index] !== undefined ? progressData["challenges"]?.[level]?.[index] : false} />
                ))}

                {quizzesByLevel[level] && quizzesByLevel[level].map((quiz, index) => {
                    const entry = quizData.find(entry => entry.quiz_id === 10*level+quiz);
                    return entry && entry.visibility ? (
                        <ChallengeButton key={`quiz${level}${quiz}_button`} link={`quiz${level}${quiz}`} label="Quiz" Icon={Pencil2Icon} active={progressData["quizzes"]?.[level]?.[index] !== undefined ? progressData["quizzes"]?.[levelStr]?.[index] : false} />
                    ) : null;
                })}
            </GridBox>
        )}
